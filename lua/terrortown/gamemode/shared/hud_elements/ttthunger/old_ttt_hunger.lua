...
if CLIENT then -- CLIENT
  ...
  function HUDELEMENT:Draw()
    ...
    if ply:IsActive() and ply:Alive() and ply:GetSubRole() == ROLE_GLUTTON then
      if not ply:GetNWInt("Appetite", 0) > 0 then
        local hungerTime = ply:GetNWInt("Hunger", 0)
    ...
    elseif multiplier then
      ...
      self:DrawComponent("Hunger", col_tbl, ply:GetNWInt("Appetite", 0) and "Hunger!", multiplier)
    end
  end
end
